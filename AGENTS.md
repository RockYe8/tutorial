...
For Python Design Intent Tree overviews, make the reasoning chain causal rather than encyclopedic: each paragraph should answer the gap opened by the previous paragraph, introduce only the concepts needed for the next step, and place observation tools such as `type(...)`, implementation details, or protocol names after the design need they verify. If a term appears in the overview, either define it in place with a short appositive or delay it until the body section where it can be fully explained. Do not use an overview to list every related mechanism; use it to prove why this chapter needs exactly these mechanisms in this order.
