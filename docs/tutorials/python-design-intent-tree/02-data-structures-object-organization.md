...
> 定义  
> 数据结构不是“更多语法”，而是把多个对象组织成一个仍然能被名字绑定、传递、返回、导入和测试的运行时对象。
...
重点不在于源码里出现了三个字符串，而在于这三个字符串对象被放进同一个顺序结构：
...
重点不在于 `tuple` 也有第 0 位、第 1 位、第 2 位，而在于这些位置的含义稳定：
...
> 解释  
> 这一区分决定了容器章节的主线：名字绑定属于命名空间，容器元素属于对象内部结构。后续函数接收容器、模块暴露容器、类属性引用容器时，传递和共享的是对象引用，不是源码里的变量名。
...
> 解释  
> 可变容器的风险不来自“名字相同”，而来自“多个路径到达同一个对象”。这条解释会在第 03 章进入函数参数边界，在第 06 章进入类属性共享边界，并在第 08 章作为可变状态的主线展开。
...
`return dollars, remainder` 返回的是一个元组。重点不在于“返回一个列表”，而在于返回一个固定组合：第 0 位是美元整数部分，第 1 位是剩余分值。调用方用解包把两个位置分别绑定到名字。
...
> 定义  
> 元素指容器内部保存的对象引用所指向的对象。异构元素指同一个组合里的元素不只按同一种规则处理：它们可以是不同类型的对象，也可以类型相同但承担不同语义角色。
...
关键不在于大括号语法，而在于查找方式。后续代码不需要记住“价格在第几个位置”，而是通过稳定键 `"cents"` 找到对应对象。
...
例如，Python 代码可以依赖 `list` 保持元素顺序，可以依赖 `dict` 按键查找值，可以依赖 `set` 去重和成员测试；这些是语言层和标准库文档承诺的行为。CPython 内部如何扩容列表、如何组织字典表、如何优化集合查找，是解释性能和内存现象的实现细节。实现细节有价值，但不能替代语言模型；结构含义应先按语言层判断，性能、共享引用或内存行为再回到 CPython 实现层解释。
