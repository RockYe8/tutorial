...
PEP 指 Python Enhancement Proposal，即 Python 增强提案。不同 PEP 的权威不同：有些提出语言变化，有些记录流程、风格或设计偏好。PEP 20 是信息型文本，概括了 Python 偏好的表达方向，例如显式、简单、扁平和可读；PEP 8 是 Python 代码风格指南，它说明代码被阅读的次数远多于被编写的次数，风格约定的作用是提高可读性，并让广泛的 Python 代码保持一致。[PEP 20][pep20][PEP 8][pep8]
...
第一层是解析器必须服从的块结构。复合语句是由多个部分组成的语句，例如 `if`、`for`、`while`、`try`、`with`、`def` 和 `class`。Python 语言参考把复合语句看成由一个或多个分句组成，官方术语是 clause；例如一条 `if` 语句可以包含 `if`、`elif` 和 `else` 三个分句。每个分句都有一个以冒号结尾的开头部分，官方术语是 header，例如 `if price > 100:`；后面跟着一组受这个开头控制的语句，这组语句的正式名称是 suite。[Python Language Reference: Compound statements][python-compound-statements]
...
块结构指一组语句在控制流里属于谁。下面这段代码里，`discount = 20` 和 `total = price - discount` 属于 `if price > 100:` 控制的区域；`print(total)` 回到了外层：
...
文档字符串是对象文档入口。它用字符串字面量写成，常见形式是三引号字符串：
...
future 语句是另一类特殊机制。它的形式是：

```python
from __future__ import feature
```

这里的 `future` 不是“未来要导入一个普通库”的意思，而是“当前模块选择启用某个未来版本才会成为标准的语言特性”。Python 语言参考把 future 语句定义为给编译器的指令：某个模块要用指定未来版本才会可用的语法或语义来编译。[Python Language Reference: Future statements][python-future]
...
默认规则，指一个模块没有写额外声明时，解释器自动采用的语言规则。历史上的 `division` 是一个清楚例子。在 Python 2 的迁移期，没有 future 语句的模块默认采用旧除法规则：两个整数相除时，`1 / 2` 得到 `0`。写了 `from __future__ import division` 的模块则提前采用未来规则：`/` 表示真除法，`1 / 2` 得到 `0.5`；需要向下取整时改用 `//`。到 Python 3 以后，真除法成为默认规则，所以普通 Python 3 模块不写 future 语句，`1 / 2` 也会得到 `0.5`。[PEP 238][pep238] 这个例子说明，“未来会变成默认规则”不是说代码现在不存在，而是说同一条源码在迁移期需要一个显式开关，等语言完成迁移后，这个开关代表的行为就成为普通默认行为。
