...
> 定义  
> 函数对象是 `def` 执行后产生的运行时对象。它保存可调用行为所需的信息，并能像其他对象一样被名字绑定、传递、放进容器或作为模块属性暴露。
...
> 定义  
> 参数绑定指函数调用时，调用表达式先取得实参对象引用，再把这些对象引用绑定到本次调用局部命名空间中的形参名字。它不是把外部变量名搬进函数，也不是复制一份完整对象。

调用表达式先通过外部名字 `products` 找到 list 对象，再把这个对象引用绑定到本次调用局部命名空间里的形参名字 `items`。

> 解释  
> 参数绑定把“同一个函数对象”与“本次调用要处理的对象”分开。函数对象可以长期存在并被多个地方引用；每次调用只临时建立一组形参到实参对象的绑定。因此，函数复用依赖的是调用现场的局部命名空间，而不是修改函数对象本身。
...
> 定义  
> 返回值是函数调用完成后交给调用表达式的结果对象。调用方可以把它绑定到名字、放进容器、继续传给别的函数，或在测试中直接断言。
...
> 定义  
> 局部命名空间是一次函数调用内部保存临时名字绑定的映射边界。它隔离的是本次调用中的名字绑定，不会自动复制或隔离传入对象本身的可变状态。
...
> 解释  
> 局部命名空间只隔离名字绑定，不隔离对象状态。若实参对象是可变容器，函数内部通过这个引用修改对象，调用方仍会看到同一个对象的变化。第 06 章的方法调用会复用同一判断：`self` 也是绑定到对象的形参名。
